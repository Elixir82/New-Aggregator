...
function NewsCard() {
  ...
  const loderRef= React.useRef(null);
  let { query } = useQuery();
  React.useEffect(() => {
  ...
  return (
    <>
      ...
      </div>) : (<div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {articles.map((article) => (
          <div
            key={article.link}
            className="bg-white shadow-md hover:shadow-xl rounded-2xl overflow-hidden border border-gray-200 transition-all duration-300 flex flex-col justify-between"
          >
            <img
            ...
            <a
              ...
              className="text-sm font-medium text-blue-600 hover:underline px-4 pb-4"
            >
              Read more →
            </a>
          </div>
